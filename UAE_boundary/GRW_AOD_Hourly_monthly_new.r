...
for (i in 1:12){
  # i=1
  load("D:/Air Quality/federico AQ index/station.RData")
...
###### monthly loop #########
#############################
coefi_conver<- 94    # this number is used to change the AOD values to PM2.5 mg/m3 obtaine by the regression of 
...
result_monthly<- list()
{
  ...
  for (qq in 1:12){
    ...
    ECMWF_OM_ECMWF_jan_10km<- raster(paste0("ECMWF/ECMWF_OM_",name_mon,"_10km.tif"))*94
    ECMWF_BC_ECMWF_jan_10km<- raster(paste0("ECMWF/ECMWF_BC_",name_mon,"_10km.tif"))*94
    ECMWF_SALT_ECMWF_jan_10km<- raster(paste0("ECMWF/ECMWF_SALT_",name_mon,"_10km.tif"))*94
    ECMWF_DUST_ECMWF_jan_10km<- raster(paste0("ECMWF/ECMWF_DUST_",name_mon,"_10km.tif"))*94
    ECMWF_SO4_jan_10km<- raster(paste0("ECMWF/ECMWF_SO4_",name_mon,"_10km.tif"))*94
    ...
    gwrG_pnt <- gwr(BIAS ~ urban_fraction + desert_fraction + ED + ECMWF_DUST_jan_10km + ECMWF_SO4_jan_10km ,#+ desert_fraction + ECMWF_SALT_jan_10km,#   , #  ECMWF_DUST_jan_10km ++ ECMWF_DUST_jan_10km + ECMWF_BC_jan_10km
                    data= combined_data_pnt_ED,  bandwidth = 15,
                    gweight = gwr.Gauss, hatmatrix = TRUE)
    }else{
      gwrG_pnt <- gwr(BIAS ~ urban_fraction + desert_fraction + ED + ECMWF_DUST_jan_10km + ECMWF_SO4_jan_10km ,#+ desert_fraction + ECMWF_SALT_jan_10km,#   , #  ECMWF_DUST_jan_10km ++ ECMWF_DUST_jan_10km + ECMWF_BC_jan_10km
...
at= unique(c(seq(-2, 0, length=100), seq(0, 10,
                                         length=100))), col.regions = colorRampPalette(c("blue", "white",
                                                                                         "red"))(1e3)) 

pal_LU_1km <-   colorQuantile(c("#0000ff", "#ffff00", "#ff0000"),
                                          getValues(Dust_coeffi),na.color = "transparent")
...
p1<- qplot( dawi$localR2, binwidth= 0.01, xlim=c(0,1),
            geom="histogram",xlab = paste0(month.name[(kk+1)]), fill=I("blue" ),alpha=I(.4),ylab ="")
plots_hist[[kk+1]]<- list(plots_hist, p1)

}
plots_hist[[2]]

multiplot(plots_hist,cols=3 )
hist( dawi$localR2, breaks= 100,xlim=c(0,1),cols=3)


}

raster_stack<- stack(BIAS, ECMWF_SO4_jan_10km)
jnk=layerStats(raster_stack, 'pearson', na.rm=T)
corr_matrix=jnk$'pearson correlation coefficient'


corr_moni_so4<-cor(r_moni, ECMWF_SO4_jan_10km,method='pearson')


names(combined_data_pnt)
ECMWF_OM_ECMWF_jan_10km+ ECMWF_BC_ECMWF_jan_10km + ECMWF_SALT_ECMWF_jan_10km+
  + ECMWF_OM_ECMWF_jan_10km+ ECMWF_BC_ECMWF_jan_10km + ECMWF_SALT_ECMWF_jan_10km+
