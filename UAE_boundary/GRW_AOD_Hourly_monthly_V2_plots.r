...
# plot(OD_mean_jan)
  for (qq in 1:12){
    ...
    bwG_pnt <- gwr.sel(BIAS ~     urban_fraction +desert_fraction + ECMWF_DUST_jan_10km,# + ECMWF_SO4_jan_10km, #  ED + urban_fraction +desert_fraction + ED ++ ECMWF_SALT_jan_10km  , #+ ECMWF_SALT_jan_10km,# + ECMWF_SALT_jan_10km + ECMWF_DUST_jan_10km, #+ desert_fraction ++ ECMWF_DUST_jan_10km +ECMWF_SO4_jan_10km++    ECMWF_DUST_jan_10km +ECMWF_DUST_jan_10km + ECMWF_SO4_jan_10km
                       data= combined_data_pnt_ED,  gweight = gwr.Gauss,
                       method="cv", verbose = F, show.error.messages = T)
    
    if (bwG_pnt < 11){
    gwrG_pnt <- gwr(BIAS ~  urban_fraction +desert_fraction + ECMWF_DUST_jan_10km,#+ ECMWF_SO4_jan_10km ,#  ED + urban_fraction +desert_fraction + ED + ECMWF_DUST_jan_10km + ECMWF_SO4_jan_10km+ desert_fraction + ECMWF_SALT_jan_10km,#   , #  ECMWF_DUST_jan_10km ++ ECMWF_DUST_jan_10km + ECMWF_BC_jan_10km
                    data= combined_data_pnt_ED,  bandwidth = 11,
                    gweight = gwr.Gauss, hatmatrix = TRUE)
    }else{
      gwrG_pnt <- gwr(BIAS ~  urban_fraction +desert_fraction +  ECMWF_DUST_jan_10km, # + ECMWF_SO4_jan_10km,# ED + urban_fraction + desert_fraction + ED + desert_fraction + ECMWF_SALT_jan_10km,#   , #  ECMWF_DUST_jan_10km ++ ECMWF_DUST_jan_10km + ECMWF_BC_jan_10km
  ...
  save(result_monthly, file="D:/Air Quality/GWR/Saves from GWR script/result_regression/result_regression_DU_UR_DE_percentage.RData")
